...
resource "aws_ecr_repository" "this" {
  for_each = local.create_private_repository ? { for name in var.repository_names : name => name } : {}
  name     = each.value
  #count = local.create_private_repository ? length(var.repository_names) : 0
  #name  = var.repository_names[count.index] # Use count.index to refer to the current index
...
#   for_each = local.create_private_repository && var.attach_repository_policy ? aws_ecr_repository.this : {}
...
#   for_each = local.create_private_repository && var.create_lifecycle_policy ? aws_ecr_repository.this : {}
...
resource "aws_ecrpublic_repository" "this" {
  for_each = local.create_public_repository ? { for name in var.repository_names : name => name } : {}
...
resource "aws_ecrpublic_repository_policy" "example" {
  for_each = local.create_public_repository ? aws_ecrpublic_repository.this : {}
